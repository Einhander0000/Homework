...
void enemyUpdate(enemyNPC* e, Player* p, float dt)
{
	...
	//printf("This is non-address of posX: %f\n", e->posX);


	float ESPEED = 1.0;

	printf("Result of min: %f\n", (float)(min(ESPEED, p->posX - e->posX)));
	...
	if ((e->posX != p->posX) && (e->posY != p->posY)){
		if (e->posX < p->posX) {
			deltaX = (float)(min(ESPEED, p->posX - e->posX));
		}
		else {
			deltaX = (float)(-max(ESPEED, p->posX - e->posX));
		}
		if (e->posY < p->posY) {
			deltaY = (float)(min(ESPEED, p->posX - e->posX));
		}
		else {
			deltaY = (float)(-max(ESPEED, p->posX - e->posX));
		}
		
		/*Uncomment this when we're done checking the addresses*/
		e->posX += deltaX * dt;
		e->posY += deltaY * dt;
...

{
	...
	while (!shouldExit) {
		...
		animDraw(&enemy1.data, enemyTex, (int)(enemy1.posX - theCamera.posX), (int)(enemy1.posY - theCamera.posY), 50, 50);

		//printf("enemy posX is: %f\n", enemy1.posX);
		/*Draws the enemy*/
		animDraw(&thePlayer.data, playerTex, (int)(thePlayer.posX - theCamera.posX), (int)(thePlayer.posY - theCamera.posY), 50, 50);
